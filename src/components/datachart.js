  ...
  statGrid(property_tax_info) {
    ...
    for (let value_code in value_keys) {
      ...
      for (let time_code in time_keys) {
        ...
        if (
          value_code === "taxrate" &&
          property_tax_info[time_code + "taxclass"] &&
          property_tax_info[time_code + "taxclass"].indexOf("1") > -1
        ) {
          val = "21.167%";
        } else if (
          value_code === "taxrate" &&
          property_tax_info[time_code + "taxclass"] &&
          property_tax_info[time_code + "taxclass"].indexOf("2") > -1
        ) {
          val = "12.473%";
        }
        if (val === undefined) {
          ...
          cols.push(<Col>{numWithCommas(val)}</Col>);
        }
      }
